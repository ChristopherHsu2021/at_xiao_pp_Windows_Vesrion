...
class VoiceEngine:
    ...
    def speak(self, text: str, temporary: bool = False):
        ...
        if prepared and _HAS_WINSOUND and not temporary:
            try:
    ...
    def play_wav(self, path: str):
        if not path or not os.path.exists(path) or not _HAS_WINSOUND:
            return
        play_path = _volume_adjusted_wav(path, config.settings.get("volume", 50))
        try:
            winsound.PlaySound(play_path, winsound.SND_FILENAME)
